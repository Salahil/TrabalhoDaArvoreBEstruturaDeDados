...
// Definição da classe principal da Árvore B
    ...
    private int t; // Ordem da Árvore B (mínimo de chaves em um nó = t-1)
    ...
    public void insert(String key) {
        ...
        if (noRaiz.chaves.size() == (2 * t - 1)) {// Verificando se dá pra colocar nesse nó.
    ...
    private void inserirOndeDisponivel(NoDaArvoreB no, String chave) {
        ...
        if (no.folhinha) {// Se ele for folha entra no if
            ...
            if (child.chaves.size() == (2 * t - 1)) {// if dentro do else
    ...
    private void dividirAHeranca(NoDaArvoreB parentNode, int childIndex) {
        ...
        parentNode.chaves.add(childIndex, child.chaves.get(t - 1));
        parentNode.filhotes.add(childIndex + 1, newChild);
        newChild.chaves.addAll(child.chaves.subList(t, 2 * t - 1));
        child.chaves.subList(t - 1, 2 * t - 1).clear();
        if (!child.folhinha) {
            newChild.filhotes.addAll(child.filhotes.subList(t, 2 * t));
            child.filhotes.subList(t, 2 * t).clear();
        }
        /*
